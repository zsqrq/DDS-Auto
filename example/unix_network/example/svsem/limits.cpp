...
main(int argc, char **argv)
{
  ...
  for (i = 0; i <= MAX_NIDS; i++) {
    sid[i] = semget(IPC_PRIVATE, 1, 0666 | IPC_CREAT);
    if (sid[i] == -1) {
      semmni = i;
  ...
  for (i = 1; i <= MAX_MEMBERS; i++) {
    semid = semget(IPC_PRIVATE, i, 0666 | IPC_CREAT);
    if (semid == -1) {
      semmsl = i-1;
  ...
  semmns = 0;
  for (i = 0; i < semmni; i++) {
    sid[i] = semget(IPC_PRIVATE, semmsl, 0666 | IPC_CREAT);
    if (sid[i] == -1) {
/* $$.bp$$ */
      /*
      ...
      for (j = semmsl-1; j > 0; j--) {
        sid[i] = semget(IPC_PRIVATE, j, 0666 | IPC_CREAT);
        if (sid[i] != -1) {
          semmns += j;
  ...
  semid = Semget(IPC_PRIVATE, semmsl, 0666 | IPC_CREAT);
...
/* $$.bp$$ */
  ...
  Pipe(pipefd);
  child = (pid_t*)Malloc(MAX_NPROC * sizeof(pid_t));
