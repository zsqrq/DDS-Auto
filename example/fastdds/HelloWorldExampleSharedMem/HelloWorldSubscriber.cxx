...
bool HelloWorldSubscriber::init() {
  ...
  if (participant_ == nullptr) {
    LOG(ERROR) << "Creating DomainParticipant Failed . ";
    return false;
  }
  ...
  if (subscriber_ == nullptr) {
    LOG(ERROR) << "Creating DomainParticipant Subscriber Failed . ";
    return false;
  }
  ...
  topic_ = participant_->create_topic("HelloWorldTopic", type_.get_type_name(),
                                      TOPIC_QOS_DEFAULT);
  if (topic_ == nullptr) {
    LOG(ERROR) << "Creating DomainParticipant Topic Failed . ";
    return false;
  }
  ...
  if (reader_ == nullptr) {
    LOG(ERROR) << "Creating DatReader Failed . ";
    return false;
  }
...
void HelloWorldSubscriber::SubListener::on_data_available(DataReader* reader) {
  ...
  if (reader->take_next_sample(hello_.get(), &info) ==
    ...
    if (info.valid_data) {
      ...
      LOG(INFO) << "Message " << hello_->message() << " " << hello_->index()
                << " RECEIVED With " << data_size
                << "(bytes) of Data. DataEnd = "
