...
CSession::CSession(boost::asio::io_context &io_context, CServer *server) :_socket(io_context),
_server(server),_b_close(false),_b_head_parse(false){
...
void CSession::Send(std::string msg, short msgid) {
  ...
  if (send_que_size > MAX_SENDQUE) {
    LOG(WARNING) << "session: " << _uuid << " send que fulled, size is " << MAX_SENDQUE;
    return ;
  }
...
void CSession::HandleWrite(const boost::system::error_code& error, CSessionPtr shared_self) {
  try {
    if (!error) {
      std::lock_guard<std::mutex> lock(_send_lock);
      LOG(INFO) << "send data " << _send_que.front()->_data+HEAD_DATA_LEN;
      _send_que.pop();
      if (!_send_que.empty()) {
        auto& msgnode = _send_que.front();
        boost::asio::async_write(_socket, boost::asio::buffer(msgnode->_data, msgnode->_total_len),
      ...
      while (bytes_transferred > 0) {
        if (!_b_head_parse) {
          if (bytes_transferred+_recv_head_node->_cur_len < HEAD_TOTAL_LEN) {
            std::memcpy(_recv_head_node->_data + _recv_head_node->_cur_len, _data+copy_len, bytes_transferred);
          ...
          std::memcpy(_recv_head_node->_data + _recv_head_node->_cur_len, _data + copy_len, head_remain);
          ...
          short msg_len = 0;
          std::memcpy(&msg_len, _recv_head_node->_data+HEAD_ID_LEN, HEAD_DATA_LEN);
          ...
          if (bytes_transferred < msg_len) {
            std::memcpy(_recv_msg_node->_data + _recv_msg_node->_cur_len, _data + copy_len, bytes_transferred);
          ...
          std::memcpy(_recv_msg_node->_data + _recv_msg_node->_cur_len, _data + copy_len, msg_len);
        ...
        std::memcpy(_recv_msg_node->_data + _recv_msg_node->_cur_len, _data + copy_len, remain_msg);
